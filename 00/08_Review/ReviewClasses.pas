...
//    Genre        :TString;
    ...
    public
     ...
     {$ifdef bThumbs}
      function ShowThumbs(const AFolder :TString) :Boolean;
      procedure CloseThumbWindow;
      procedure ThumbSyncDelayed(aCmd, aDelay :Integer);
      procedure ThumbChangePath(const APath, ACurFile :TString);
      procedure ThumbCommand(ACmd :TCmdObject);
     {$endif bThumbs}
...
//    FCursor        :HCursor;
     ...
     {$endif bThumbs}
      ...
      procedure DecodeImage(aImage :TReviewImage; ASize :TSize; AFastScroll :Boolean = False; APrecache :Boolean = False);
...
//    procedure WaitCursor(ASet :Boolean);
      ...
      procedure MyDecodeCallback(ADecoder :TReviewDecoder; APercent :Integer; var AContinue :Boolean);
...
//  Invalidate;
  ...
  procedure TImageWindow.SetTempMsg(const AMsg :TString; aInvalidate :Boolean = True);
  begin
    FTempMsg := AMsg;
  ...
  procedure TImageWindow.InvalidateMsgRect;
  var
    vRect :TRect;
  begin
    vRect := ClientRect;
...
//    TraceF('Draw. DstRect=%dx%d-%dx%d', [Left, Top, Right, Bottom]);
  ...
  procedure TImageWindow.DrawTempText(DC :HDC; const AStr :TString);
  var
    X, Y :Integer;
    vSize :TSize;
...
//    TraceF('Try decoding by %s - %s...', [ADecoder.Name, FName]);
  ...
  begin
    ...
    { Декодируем... }
    if FDecoder.pvdGetPageInfo(Self) and FDecoder.pvdPageDecode(Self, 0{???}, ASize.CX, ASize.CY, AFastScroll) then begin
 ...
 {-----------------------------------------------------------------------------}
  ...
  begin
    ...
    DecodeCallback := MyDecodeCallback;
  end;
...
//end;
  ...
  procedure TReviewManager.MyDecodeCallback(ADecoder :TReviewDecoder; APercent :Integer; var AContinue :Boolean);
  begin
    ...
    if (FDecodeStart <> 0) and (TickCountDiff(GetTickCount, FDecodeStart) > FDecodeStep) then begin
      ShowDecodeProgress(ADecoder, APercent);
  ...
  procedure TReviewManager.DecodeImage(aImage :TReviewImage; ASize :TSize; AFastScroll :Boolean = False; APrecache :Boolean = False);
  begin
//  WaitCursor(True);
//  FScreen := 0;
    FOldTitle := '';
...
//    TraceF('ProcessWheel: %d', [ADelta]);
 ...
 {$ifdef bThumbs}
  function TReviewManager.ShowThumbs(const AFolder :TString) :Boolean;
  var
    vThumbs :TObjList;
    vRec :TSetThumbsRec;
...
//    Trace('ThumbSyncDelayed...');
  ...
  procedure TReviewManager.ThumbChangePath(const APath, ACurFile :TString);
  begin
  ...
  procedure TReviewManager.ThumbCommand(ACmd :TCmdObject);

    procedure LocSetCurrent;
    begin
      FarPanelSetCurrentItem(True, ACmd[0]);  {!!!Оптимизировать... }
      if ThumbsModalDlg <> nil then
        ThumbsModalDlg.UpdateTitle;
    end;

    procedure LocSelect(ASelect :Boolean = True);
    var
    ...
    begin
     ...
     {$endif bDebug}
      ...
      if FarGetPanelInfo(PANEL_ACTIVE, vInfo) and (vInfo.PanelType = PTYPE_FILEPANEL) then begin
        ...
        try
          vCount := 0;
          ACmd.SortList(True, 0);
          for I := 0 to vInfo.ItemsNumber - 1 do begin
            vName := FarPanelItemName(PANEL_ACTIVE, FCTL_GETPANELITEM, I);
            if ACmd.FindKey(Pointer(vName), 0, [foBinary], vIndex) then begin
              FARAPI.Control(PANEL_ACTIVE, FCTL_SETSELECTION, I, Pointer(TIntPtr(IntIf(ASelect, PPIF_SELECTED, 0))) );
              Inc(vCount);
              if vCount = ACmd.Count then
                Break;
            end;
          end;
        finally
  ...
  begin
    case ACmd.FCommand of
      CmdSetFile  : LocSetCurrent;
      CmdSelect   : LocSelect(True);
      CmdDeselect : LocSelect(False);
      CmdGoFolder : ThumbChangePath(ACmd[0], '');
    end;
    FreeObj(ACmd)
  end;
 {$endif bThumbs}
